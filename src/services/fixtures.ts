...
function normalizeMatch(value: unknown, index: number): Fixture {
  ...
  const fullTime = object(
    object(match.score, `matches[${index}].score`).fullTime,
    `matches[${index}].score.fullTime`,
  );
  const matchweek = match.matchday;
  if (!Number.isInteger(matchweek) || (matchweek as number) < 1)
    throw new Error(`matches[${index}].matchday is invalid.`);
  const kickoff = string(match.utcDate, `matches[${index}].utcDate`);
  if (Number.isNaN(Date.parse(kickoff)))
    throw new Error(`matches[${index}].utcDate is invalid.`);
  const sourceStatus = string(match.status, `matches[${index}].status`);
  const statuses: Record<string, Fixture["status"]> = {
    FINISHED: "finished",
    SCHEDULED: "scheduled",
    TIMED: "scheduled",
    POSTPONED: "postponed",
  ...
  const homeScore = score(
    fullTime.home,
    `matches[${index}].score.fullTime.home`,
  );
  const awayScore = score(
    fullTime.away,
    `matches[${index}].score.fullTime.away`,
  );
  if (status === "finished" && (homeScore === null || awayScore === null))
    throw new Error(`Finished match ${index} must include both scores.`);
  if (status !== "finished" && (homeScore !== null || awayScore !== null))
    throw new Error(`Unplayed match ${index} cannot include a score.`);

  if (
    (typeof match.id !== "number" && typeof match.id !== "string") ||
    String(match.id).length === 0
  )
    throw new Error(`matches[${index}].id is invalid.`);
  return {
    id: String(match.id),
    kickoff,
    matchweek: matchweek as number,
...
export async function fetchFixtures(signal?: AbortSignal): Promise<Fixture[]> {
  ...
  if (!Array.isArray(payload.matches))
    throw new Error("Fixture response must contain a matches array.");
